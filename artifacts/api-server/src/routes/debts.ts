  ...
  const rows = db.prepare(`
    SELECT c.id, c.name, c.phone, c.area,
      COALESCE(c.openingBalance, 0)
        + COALESCE(SUM(CASE WHEN si.status IN ('finalized','partially_paid','credit') THEN si.remainingAmount ELSE 0 END), 0)
  ...
  const rows = db.prepare(`
    SELECT s.id, s.name, s.phone,
      COALESCE(s.openingBalance, 0)
        + COALESCE(SUM(CASE WHEN pi.status IN ('finalized','partially_paid','credit') THEN pi.remainingAmount ELSE 0 END), 0)
