# AGENTS.md — Local‑First POS (نظام المحل)

> Handoff guide for the next AI/dev agent. **Read this fully before editing.**
> Arabic‑first (RTL) **local‑first** POS & inventory system for an Egyptian
> sanitary‑ware / paints / plumbing shop. Target runtime: a **Windows** shop PC.
> Development: **macOS**. Database: **local SQLite**. No cloud, no paid hosting.

---

## 0. TL;DR — the 6 things that bite people

1. **Use Node 24.** The API server uses the built‑in `node:sqlite`, which only
   exists on Node ≥ 22.5. The machine's default `node` is **v20** → crashes with
   `No such built-in module: node:sqlite`. Run `nvm use 24` (there is an `.nvmrc`).
2. **The DB is SQLite via `node:sqlite`, NOT Postgres.** `lib/db` (Drizzle/PG) is
   unused template scaffolding. The real schema is `artifacts/api-server/src/lib/db.ts`.
3. **The backend is hand‑written; the frontend uses generated hooks from an
   OpenAPI spec.** If you change a route's shape, update `lib/api-spec/openapi.yaml`
   and re‑run codegen, or the POS typecheck breaks.
4. **The whole UI is RTL/Arabic.** Radix needs `<DirectionProvider dir="rtl">`
   (in `App.tsx`) — without it, tables inside `Tabs` render left‑to‑right.
5. **pnpm is strict (isolated node_modules).** A package can only import its
   *direct* deps. Add new libs to that package's `package.json` + `pnpm install`.
6. **Don't commit `data/store.db*`.** It holds local/seed/test data. Stage source
   files only.

---

## 0.5 Production / shop-mode facts (verified on the real Windows shop PC)

The app was **installed and run successfully on the actual Windows shop machine**
(Node 24 + Git + pnpm), demo/seed data was safely cleaned (backup first, DB **not**
deleted, schema unchanged, dashboard totals went to zero), and daily operation works.
Keep these facts intact:

- **Local-first POS** — one local machine, no cloud. **Shop URL = `http://localhost:8080`**
  (health: `http://localhost:8080/api/healthz` → `{"status":"ok"}`).
- **Shop mode = one origin on `8080`.** The **API server serves both `/api/*` and the
  built frontend** (`artifacts/pos/dist/public`). The owner does **not** use `:24730`.
- **`:24730` is development mode only** (separate Vite dev server, proxies `/api` → 8080).
  Dev can still run the two-server split; the shop never does.
- **Daily use = one double-click** on the desktop shortcut **«نظام المحل»** (runs
  `Start POS Hidden.vbs` → `Start POS.bat`, opens the browser). The **owner is
  non-technical**: no terminal, no commands, no ports. If it won't open → contact
  admin/developer.
- **Launcher scripts** (root): `Setup POS.bat` (first-time admin setup),
  `Start POS.bat` / `Start POS Hidden.vbs` (daily one-click, won't start a 2nd server if
  8080 is already listening, logs to `logs/pos-server.log`), `Stop POS.bat` (admin/support),
  `Update POS.bat` (admin — **backs up `data/store.db` before `git pull`**),
  `Create Desktop Shortcut.vbs` (makes the «نظام المحل» shortcut).
- **`data/store.db`** is the local SQLite DB (created on first boot). **Never delete it.
  Never commit the real shop DB or backups or logs** (`.gitignore` covers `data/*.db`,
  `backups/`, `logs/`, `PRODUCTION_DATA_CLEANUP_NOTE.txt`). **Always back up before updates.**
- **Do not change invoice / inventory / debt / customer / craftsman / supplier / report
  business logic** unless explicitly asked.
- Staff usage-support prompt for a separate ChatGPT project: `docs/SHOP_CHATGPT_ASSISTANT_PROMPT_AR.md`.

> **Local-dev note (macOS):** newer pnpm (via corepack) escalates
> `ERR_PNPM_IGNORED_BUILDS` to a hard error in its pre-run deps check, which broke
> `pnpm run dev`/`build:shop`. Fixed in `pnpm-workspace.yaml` → `allowBuilds:`
> (`esbuild: true`, `better-sqlite3: false` — the latter is an unused leftover dep; the
> app uses built-in `node:sqlite`). Don't let pnpm rewrite those back to the
> `set this to true or false` placeholder.

---

## 1. How to run locally (verified working)

```bash
# one terminal — Node 24 required for node:sqlite
nvm use 24                 # or: export PATH="$HOME/.nvm/versions/node/v24.14.0/bin:$PATH"

# backend → http://localhost:8080  (health: /api/healthz)
PORT=8080 pnpm --filter @workspace/api-server run dev

# another terminal — frontend → http://localhost:24730
PORT=24730 BASE_PATH=/ pnpm --filter @workspace/pos run dev
```

- The Vite dev server **proxies `/api` → `http://localhost:8080`** (configured in
  `artifacts/pos/vite.config.ts`, *local only* — disabled when `REPL_ID` is set so
  Replit's own router is used). So the SPA and API share one origin in the browser.
- `PORT` **and** `BASE_PATH` are **required** by `vite.config.ts` (it throws if
  missing) — even for `pnpm --filter @workspace/pos run build`.
- `node:sqlite` prints an `ExperimentalWarning` — harmless.
- Build commands: `pnpm run typecheck` (all packages), `pnpm run build`,
  `pnpm --filter @workspace/api-spec run codegen` (regenerate client + zod).

---

## 2. Architecture & stack

pnpm workspace (Node 24, TypeScript 5.9). Packages:

| Package | What | Notes |
|---|---|---|
| `artifacts/api-server` | Express 5 API, bundled by esbuild → single ESM | **DB = `node:sqlite`** |
| `artifacts/pos` | React 19 + Vite 7 SPA | Tailwind v4, Radix UI, wouter, TanStack Query, framer‑motion, sonner, jsbarcode |
| `lib/api-spec` | OpenAPI (`openapi.yaml`) → **Orval** codegen | source of API types |
| `lib/api-client-react` | generated React‑Query hooks | **do not hand‑edit `generated/`** |
| `lib/api-zod` | generated Zod schemas | regenerated by codegen |
| `lib/db` | Drizzle + Postgres scaffolding | **UNUSED** (leftover template) |
| `artifacts/mockup-sandbox` | unrelated UI sandbox | ignore |

### Where things live
- **DB schema + helpers (truth):** `artifacts/api-server/src/lib/db.ts`
- **API routes:** `artifacts/api-server/src/routes/*.ts` (mounted under `/api` by `routes/index.ts`)
- **SPA pages:** `artifacts/pos/src/pages/*.tsx`
- **Sidebar/layout:** `artifacts/pos/src/components/layout.tsx`
- **Shared UI (shadcn):** `artifacts/pos/src/components/ui/*`
- **Barcode scanner hook:** `artifacts/pos/src/hooks/use-barcode-scanner.ts`
- **OpenAPI spec (edit this, then codegen):** `lib/api-spec/openapi.yaml`
- **SQLite file:** `data/store.db` (committed seed; schema auto‑created on boot)

---

## 3. The API contract (how the frontend talks to the backend)

- Frontend imports **hooks** from `@workspace/api-client-react` (e.g.
  `useListSalesInvoices`, `useGetCraftsman`). It also imports **imperative async
  functions** for event handlers (e.g. `searchProducts(...)`,
  `lookupSalesInvoiceBySerial(...)`).
- The backend is **hand‑written** and **not** generated from the spec. The spec is
  kept in sync **manually**.
- **Procedure when you change a route's request/response shape:**
  1. Edit the route in `artifacts/api-server/src/routes/*.ts`.
  2. Mirror the change in `lib/api-spec/openapi.yaml`.
  3. `pnpm --filter @workspace/api-spec run codegen` (regenerates client + zod, then typechecks libs).
  4. Update frontend usages; run `pnpm --filter @workspace/pos run typecheck`.
- **All list endpoints return `{ items, total }`** (every `*Page` schema). Reports
  use `dateFrom` / `dateTo` query params.

---

## 4. Database notes

- WAL mode. `initializeSchema()` runs on **every** startup using
  `CREATE TABLE/INDEX IF NOT EXISTS` → **idempotent**. To add a table/index, just
  add it there; it auto‑applies on next boot (existing columns are NOT altered —
  `IF NOT EXISTS` won't add a column to an existing table, so for new columns you
  must migrate explicitly).
- Serials come from `nextSerial(prefix)` using counters in the `settings` row.
- `data/store.db-shm` / `-wal` are **transient** — they show up as "modified" a lot.
  Don't commit them (ideally gitignore them; currently tracked).

---

## 5. Domain rules (don't break these)

- **Craftsman ↔ Customer linking:** a sales invoice / quotation may carry **both**
  an optional `customerId` and an optional `craftsmanId` (the introducer/worker).
- **Debt belongs to the CUSTOMER**, never the craftsman. `remainingAmount` is the
  customer's debt and feeds the debts page. Craftsman **commission**
  (`sales_invoices.craftsmanCommission`, % from `craftsmen.commissionPercent`) is
  tracked **separately**. **Never mix** customer debt / supplier debt / craftsman commission.
- **Quotation → invoice convert** preserves both ids, sets `invoice.quotationId`
  and `quotation.convertedInvoiceId`.
- **Pricing:** `netPurchase = listPrice × (1 − supplierDiscount%)`,
  `trueCost = net + extraCost`, `profit = sellingPrice − trueCost`.
- `/craftsmen/:id/customers` uses **correlated subqueries** (NOT a dual LEFT JOIN)
  to avoid aggregate fan‑out — keep it that way.
- **`products.soldByWeight`**: `0` = sold by piece (gets a barcode + printable stickers,
  integer qty); `1` = sold by weight/measure (NO barcode, fractional qty, prices are
  per‑unit e.g. “بالكيلو”). A product with an **empty barcode is never printed**.

---

## 6. UI conventions

- **RTL + Arabic everywhere.** `index.html` = `<html lang="ar" dir="rtl">`, plus CSS
  `html, body { direction: rtl }`.
- **Radix needs RTL too:** the app is wrapped in `<DirectionProvider dir="rtl">`
  (`App.tsx`, from `@radix-ui/react-direction`). Removing it makes Radix `Tabs`
  emit `dir="ltr"`, which flips any `<table>` inside a tab to left‑to‑right.
- Money → `formatCurrency` (`src/lib/format.ts`). Animations → framer‑motion.
- Tailwind v4 + shadcn‑style components in `components/ui`.

---

## 7. Session changelog (everything done so far)

### Session 2026‑07‑19 — system‑wide delete, cashier UX, server auth + adversarial review (COMMITTED & PUSHED)
Committed to `main` and pushed. `pnpm run typecheck` and `pnpm run build` are green. Restart the API after backend edits (`dev` = build && start). Verified end‑to‑end in the browser.

**Accounting‑aware delete everywhere** (`lib/deletion.ts`, `routes/delete-preview.ts`, `components/ui/delete-confirm.tsx` + every list page)
- One module `lib/deletion.ts` exposes `previewDelete()` + `performDelete()`; the confirm dialog renders the server preview **verbatim** and the DELETE handler runs the *same* logic, so they can't drift.
- `GET /delete-preview/:entity/:id` → `{ canDelete, mode: 'hard'|'archive', effects[], warnings[], blockers[] }` in Arabic. Delete buttons on quotations, sales, returns, purchases, products, customers, craftsmen, suppliers, categories, brands, units.
- Stock reversal is **movement‑based** (`Σ balanceAfter−balanceBefore` per product for the doc), so it's correct for every status; reversals are logged as documented `adjustment` movements. Master data: hard‑delete when unreferenced, **archive** (`isActive=0`) when history exists, **block** when money is outstanding (customer/supplier balance, unpaid commission, linked returns, paid‑out commission, attribute still used by products). New `DELETE /returns/:id` reverses restocks/exchange stock/account settlement.

**Cashier & product‑form UX** (`pages/pos.tsx`, `pages/products.tsx`)
- Line **discount accepts negatives** (= surcharge); **line total is directly editable** (back‑computes the discount); new `NumericInput` keeps partial entries ("-", "5.") alive while typing. Editable **quantity** field (by‑piece stays integer, by‑weight allows fractions).
- Product form: **Enter acts like Tab** (jumps to next field, skips read‑only computed costs); «خصم المورد %» starts empty (was hardcoded 30).

**Server‑side auth (was WIDE OPEN)** (`app.ts`, `middlewares/auth.ts`, `lib/session.ts`, `routes/auth.ts`, `custom-fetch.ts`, `App.tsx`)
- Login now issues an opaque token persisted in a new **`sessions`** table (survives restarts). `requireAuth` gates every `/api` request (only `POST /auth/login` + `GET /healthz` public → else 401). `requireAdmin` gates `/users`, `/backup`, settings writes. Client sends `Authorization: Bearer`; a fetch‑layer `setUnauthorizedHandler` drops a stale session on any 401 and returns to login (upgraded installs whose stored user predates tokens are cleanly re‑prompted). **Passwords stay plaintext (deliberate manager‑view feature) but are now admin‑gated.**

**Adversarial multi‑agent review → 47 of 49 confirmed bugs fixed**
- **Transactions**: sales/purchases/quotations POST+PATCH+duplicate and stock‑count creation now wrapped in BEGIN/COMMIT.
- **`POST /sales/:id/cancel`** now reverses stock (from movements) + applies the same blockers as delete (was flipping status only → orphaned stock + vanished debt/commission).
- **Debt formulas** include the `'paid'` status everywhere (overpaid‑invoice customer credit no longer disappears).
- **Dashboard/reports**: cash = money actually collected (incl. partial), credit = owed → tiles reconcile with total; sales report nets in returns/refunds; **local‑date** month range (was UTC‑shifted); by‑category report LEFT JOINs (uncategorized sales appear); commission report keeps archived craftsmen with in‑range activity; statement sorts same‑day payments after that day's invoices.
- **Quotations**: convert assigns `partially_paid` correctly + refuses cancelled; PATCH keeps the stored discount on an items‑only edit.
- **Purchases**: finalize/PATCH keep a recorded non‑cash `paidAmount`; `remainingAmount` always re‑derived.
- **Commission payout**: compares against the 2‑dp‑rounded outstanding and lets a full payout absorb sub‑cent residue (full withdrawal of a fractional balance settles to exactly 0).
- **Archived entities hidden by default** from customers/craftsmen/suppliers/products lists (matches the "سيتم إخفاؤه" promise) and from POS/quotation/purchase pickers (`isActive:true`). Commission‑paid blocker checks `craftsman_payout_items` existence (no raw FK 500). A product/customer held in a cashier draft **archives** instead of hard‑deleting (FK‑safe on resume). Warns when deleting a settled credit invoice would flip the customer to credit.
- **API spec**: declared the `isActive` filter, renamed `from/to`→`dateFrom/dateTo` (what the backend reads), `page`→`offset` pagination, added session `token` to the login response; regenerated the client + zod.
- **Deliberately deferred**: returns→craftsman‑commission adjustment (a proper reversible fix is a feature, not a bug fix — a partial fix would introduce ledger asymmetry).

**Migrations/tables added** (`lib/db.ts`): new **`sessions`** table auto‑creates on boot.

### Session 2026‑06‑29 — returns, users/RBAC, commission payouts, PDF archive, full bug‑fix pass (UNCOMMITTED)
All on the working tree (not committed). `pnpm run typecheck` and `pnpm run build:shop` are green. Restart the API after backend edits (`dev` = build && start).

**New features**
- **Purchases drafts are now usable** (`pages/purchases.tsx`, `routes/purchases.ts`): click a purchase row → opens it; a **draft** can be edited and **finalized** (stock goes up) or **deleted**; a finalized one opens **read‑only** with **«إعادة طباعة الباركود»**. Bigger dialog; product search is a floating dropdown; columns distributed (`table-fixed`).
- **Barcode stickers on purchase** (`pages/purchases.tsx` + `lib/print-labels.ts`): finalizing a purchase prints one sticker per added unit (copies = quantity); items with no barcode (sold‑by‑weight) are skipped; per‑row print toggle.
- **Craftsman commission payouts / سحب العمولة** (`pages/craftsmen.tsx`, `routes/craftsmen.ts`, `lib/print-commission.ts`): new `craftsman_payouts` + `craftsman_payout_items` tables; outstanding = earned − paid (`sales_invoices.commissionPaid` column); **partial** withdrawals allocate FIFO across the craftsman's unsettled invoices; prints an A4 receipt; payouts history tab + reprint. New serial `CMP-YYYY-…` via `nextPayoutSerial()` (counters table).
- **Returns & Exchange / المرتجعات والاستبدال** (new `pages/returns.tsx`, `routes/returns.ts`, `lib/print-return.ts`): scan/enter an invoice serial → per‑item return by qty (capped by already‑returned), optional exchange items, **سليم/تالف** restock toggle, **cash or on‑account** settlement; net = newItems − returned. New `returns` + `return_items` tables; serial `RET-YYYY-…`. **Account‑settled returns are folded into customer debt** (debts list, customers.totalDebt, statement). Stock: returned (restock) ↑, exchange items ↓.
- **Users page + per‑page RBAC** (new `pages/users.tsx`, `routes/users.ts`, `lib/pages.tsx`): admin‑only page; create users, view/change plaintext passwords, role (admin/staff), and a **permissions dialog** (Switch per page). `users.permissions` (JSON array of allowed route keys); `role='admin'` = full access. Central **`lib/pages.tsx`** registry now drives nav (`layout.tsx`) **and** routes (`App.tsx`); non‑admins only see/reach their allowed pages (catch‑all `<Redirect>` to first allowed). Login returns `permissions`. **Permissions apply on next login only** (accepted by the owner). ⚠️ **Superseded 2026‑07‑19:** server‑side session auth now exists (see the 2026‑07‑19 entry) — `requireAuth` gates all `/api`, `requireAdmin` gates users/backup/settings‑writes. Per‑page RBAC is still enforced in the UI only (server enforces authenticated + admin, not per‑page keys).
- **Auto‑archive every document to PDF** (new `lib/archive.ts`, `lib/pdf.ts`, `lib/dialog.ts`, `routes/backup.ts`, `pages/settings.tsx`): pick a folder in Settings → every finalized sale/purchase/return/payout is written as a **PDF** into `العملاء/<customer>/`, `الصنايعية/`, `المشتريات/`. PDF is produced by driving the **installed system browser headless** (`--headless --print-to-pdf`, found via `findBrowser()`) — no bundled Chromium; correct Arabic. `htmlToPdf` polls for the output file (modern Chrome/Edge don't exit after print) then kills the browser. Archive dir is read **fresh** (`getArchiveDir()` in `paths.ts`, stored in `pos-config.json`) so it applies without restart. **Native folder‑picker** (`dialog.ts`: PowerShell FolderBrowserDialog on Windows / `osascript` on mac) via `POST /api/backup/pick-folder` → Settings «تصفّح» buttons fill the path. `/backup/info` now also returns `archiveDir` + `pdfEngineAvailable`.
- **Toast notifications restyled** (`App.tsx` sonner + `index.css`): top‑left, app font, slide in/out behind the left edge.

**Bug‑fix pass (adversarial multi‑agent review → 19 confirmed bugs fixed)**
- **Customer & supplier payments now reduce the debt** (`routes/debts.ts`, `customers.ts`, `suppliers.ts`, `dashboard.ts`): all debt aggregates subtract `customer_payments`/`supplier_payments` (they previously only summed invoice `remainingAmount`, so recorded سداد payments were ignored). Dashboard receivables/payables now use the **same per‑entity positive‑balance formula** as `/debts/*` (incl. opening balance + account returns − payments).
- **Sales PATCH** recomputes `total`/`remainingAmount` on every path (a discount/payment‑only edit no longer leaves stale money); line/invoice totals clamped `≥ 0` (no negative invoices from over‑discount) — client also clamps in `pages/pos.tsx`.
- **Sales finalize status**: any positive remaining on a non‑credit sale → `partially_paid` (was mislabeled `finalized` when paid = 0).
- **Products PATCH** can now clear nullable text fields (barcode/nameEn/notes/…); empty string → NULL instead of COALESCE keeping the old value; **soldByWeight ⇒ barcode forced NULL** (domain rule).
- **Purchases PATCH/DELETE** now reject non‑draft invoices (were editable/cancellable after finalize, corrupting stock); **finalize** no longer wipes `products.trueCost` to 0 on a 0‑cost line (split stock‑increment from cost‑update; only set cost when `> 0`).
- **Customer statement** and **`/craftsmen/:id/customers` totalRemaining** now exclude `cancelled` invoices (matched the debt screens).
- **Settings PATCH** can clear `shopPhone`/`shopAddress`.
- **PDF**: use `pathToFileURL()` (raw `file://C:\…` is not loadable on Windows). **`safeName`** keeps hyphens so archived filenames match serials (`INV-2026-…`).
- **POS draft autosave** now includes `paidAmount` (a restored partial draft no longer finalizes with 0 paid).
- **Purchases**: re‑scanning an added product bumps its qty (was a silent no‑op); qty field no longer snaps empty→1 mid‑edit.
- **Error handling**: `craftsmen.tsx` add and `settings.tsx` save now `try/catch` with a toast.
- **OpenAPI** `BackupInfo.sizeBytes` → `size` (matched the backend); regenerated client.

**Migrations added** (`ensureColumn` in `lib/db.ts`): `sales_invoices.commissionPaid`, `users.permissions`. New tables auto‑create on boot: `craftsman_payouts`, `craftsman_payout_items`, `returns`, `return_items`. New counter‑based serials: `commission_payout` (CMP), `return` (RET).

### Session 2026‑06‑27 — products overhaul, barcodes, weight‑sale (UNCOMMITTED)
All of the below is on the working tree only (not committed/pushed yet). Typecheck
+ pos build are green. The API `dev` script is `build && start` (NOT watch) — after
editing `artifacts/api-server/**` you MUST restart it for changes to take effect.

- **Add‑product dialog rebuilt** (`pages/products.tsx`). Wide dialog (`max-w-[1340px]`,
  `w-[96vw]`); two halves — identity on the **right**, pricing (حسبة الأسعار) on the
  **left**. **Batch add**: a shared `ProductFormFields` feeds a queue/sheet; “إضافة
  المنتج للقائمة” pushes the current entry, then **“حفظ الكل وطباعة”** saves the whole
  batch and prints stickers. Per‑row actions: تعديل / حذف / **طباعة‑أو‑عدم‑طباعة** toggle.
  Save loop is recoverable: on a mid‑batch failure, already‑saved rows are dropped from
  the queue so a retry doesn’t double‑save.
- **Edit existing products**: click any product row → edit dialog (PATCH). Reuses
  `ProductFormFields`. **`currentStock` is read‑only on edit** (stock only moves via
  purchases/جرد; the PATCH route ignores it anyway). Edit payload uses
  `buildPayload(form, /*skipEmpty*/ true)` so clearing a numeric input sends `undefined`
  (backend COALESCE keeps the old value) instead of zeroing the price.
- **Barcodes** (the big one):
  - Two modes in the form: **باركود جاهز** (type manually OR hardware‑scan fills it) and
    **إنشاء باركود** (generate a unique store code + live preview: bars + number + SKU).
  - Generation: backend `nextBarcode()` (`lib/db.ts`) using a new idempotent **`counters`**
    table; numbers are `"2" + 6‑digit` and skip any already in `products`. Endpoint
    **`GET /products/next-barcode`** (registered BEFORE `/products/:id`). Client fn
    `getNextBarcode()`.
  - Reusable `components/ui/barcode.tsx` (JsBarcode CODE128). 
  - **Label printing** `lib/print-labels.ts`: opens a dedicated print window (isolated
    from the A4 invoice CSS), `@page { size: 40mm 15mm; margin: 0 }`, **one sticker per
    page**, **copies = quantity (currentStock)**, a **blank separator page between
    products** (none after the last — uses `:last-of-type`, and the `<script>` lives in
    `<head>` so it doesn’t become the body’s last child).
  - A product with an **empty barcode never prints** (filtered out; no separator either).
- **Sale type — count vs weight** (`soldByWeight`):
  - New `products.soldByWeight INTEGER DEFAULT 0` + **idempotent migration** `ensureColumn()`
    in `lib/db.ts` (runs `ALTER TABLE` only if the column is missing — needed because
    `CREATE TABLE IF NOT EXISTS` won’t add columns to an existing DB). Wired through
    POST/PATCH, OpenAPI (`Product`/`ProductInput`/`ProductUpdate`), and codegen.
  - Form has a **طريقة البيع** toggle: **بالقطعة (عدد)** vs **بالوزن / الكمية**. Weight mode
    **hides the barcode block** (so weight products get no barcode/sticker) and allows a
    fractional quantity.
  - Every price/quantity field shows a unit hint: **(بالقطعة)** for count, **(بالكيلو)** for
    weight (dynamic from the chosen وحدة القياس → بالمتر/باللتر…; defaults قطعة/كيلو).
    خصم المورد % is intentionally left without a hint. Quantity label is **العدد** for count,
    **الكمية** for weight.
- **Arabic→Western digits, system‑wide**: `toWesternDigits()` (`lib/format.ts`) wired into
  the shared `components/ui/input.tsx`; numeric inputs render as `type=text inputMode=decimal`
  so Arabic numerals (٠‑٩) land and are normalised to `0‑9` everywhere. (Textarea/notes not
  yet covered.)
- **Min‑selling‑price soft warning** in the cashier (`pages/pos.tsx`): when a line’s
  discount drops the unit price below `minSellingPrice`, a yellow “السعر أقل من الحد
  المسموح” note + amber discount border appears — **warning only, does not block**. (Not
  yet wired into quotations.)
- **Suppliers page → “الموردين والماركات”** (`pages/suppliers.tsx`, sidebar label in
  `components/layout.tsx`): two **slide‑animated tabs** (`components/ui/tabs.tsx`) — موردين
  (unchanged) + **ماركات** CRUD (`useListBrands/useCreateBrand/useUpdateBrand/useDeleteBrand`).
- **Backend hardening**: POST/PATCH `/products` now translate a SQLite `UNIQUE` violation
  into a friendly Arabic **400** (`uniqueErrorMessage()`), instead of leaking an HTML 500.
- Removed the English‑name field from the add form; barcode moved to the last identity field.
- Reviewed by an adversarial multi‑agent pass; **5 confirmed bugs were found and fixed**
  (trailing blank label page, batch barcode de‑dup, non‑atomic save recovery, edit
  price‑zeroing, UNIQUE→500). 

#### ▶ NEXT (pick up here tomorrow)
1. **Sell‑by‑weight in the cashier** (`pages/pos.tsx`): for `soldByWeight` products the cart
   currently only does ±1 — let the cashier type a **decimal quantity** (e.g. 2.5 كيلو).
   The product row carries `soldByWeight`; gate the qty input on it.
2. Optional: default وحدة القياس to “كيلو” when switching a product to weight mode.
3. Extend the min‑selling‑price warning to **quotations**.
4. Normalise Arabic digits in the **Textarea** (notes) too, for full coverage.
5. Commit this session (it’s all uncommitted) once verified in the real app, and tighten
   the remaining `as any` casts in `pages/products.tsx`.

### Already on `origin/main` (pushed)
- `50412a7` — **mac/windows native‑binary fix + craftsman‑customer linking.**
  Removed the `pnpm-workspace.yaml` overrides that stripped esbuild/rollup/
  lightningcss/@tailwindcss‑oxide binaries (so it builds on macOS/Windows). Added
  DB indexes (`idx_sales_craftsman`, quotation indexes); fixed the
  `/craftsmen/:id/customers` aggregate fan‑out; fixed broken customer/supplier
  payment calls (`{customerId}`→`{id}`); added the local Vite `/api` proxy;
  `.nvmrc`=24; relaxed `noImplicitReturns` for the API package; wrote `replit.md`.
- `fdf37cd` — **OpenAPI spec aligned with the backend + regenerated client.**
  `*Page` schemas `{data,page,limit}`→`{items,total}`; reports `from/to`→
  `dateFrom/dateTo`; `QuotationInput.status`, optional `QuotationConvertInput.priceMode`,
  `PurchaseInvoiceInput.status`, `StockAdjustmentInput` `reason`→`type`; added
  `/customers/{id}/invoices`, `/customers/{id}/quotations`, `/craftsmen/{id}/customers`;
  replaced raw `fetch()` with typed hooks; fixed remaining POS typecheck errors.
- `d9c4832` — **sidebar active‑item sliding highlight** (framer‑motion `layoutId`
  in `components/layout.tsx`).

### Pending in the CURRENT commit (this session, not yet pushed when written)
- **RTL fix (root cause).** Radix `Tabs` forced `dir="ltr"`, so tables inside tabs
  (المخزن / التقارير / المديونيات / ملفات العملاء والصنايعية) rendered LTR. Fix:
  `<DirectionProvider dir="rtl">` in `App.tsx`, `<html lang="ar" dir="rtl">`, added
  `@radix-ui/react-direction`.
- **Tabs sliding active‑indicator.** `components/ui/tabs.tsx`: a single measured
  `motion.div` slides under the active trigger (works in RTL; uses
  MutationObserver + ResizeObserver to track the active tab).
- **Inventory column alignment.** `pages/inventory.tsx`: the جلسات الجرد table had a
  `text-center` cell among right‑aligned columns → unified.
- **Professional invoice print.** `pages/sales.tsx` (redesigned `InvoiceDetail` +
  a `Barcode` component using **jsbarcode**), `src/index.css` (print isolation),
  `components/ui/dialog.tsx` (`no-print` on overlay + close button). Prints clean
  A4 with: shop name/phone/address (from settings), **Code128 barcode of the
  invoice serial**, customer + craftsman, line items with **per‑item discount** and
  **total discount**, totals (subtotal/discount/total/paid/remaining), and
  **signature lines** (shop owner + customer/craftsman).
- **Laser barcode scanner — global, "scan from anywhere".**
  `hooks/use-barcode-scanner.ts` (`useBarcodeScanner` + `findProductByCode`). A
  scanner = a very fast keystroke burst ending in Enter; detected by timing
  (`maxGapMs` 50, `minLength` 3) on a `window` keydown listener, so the cashier
  never has to focus a field. Wired into:
  - **POS** (`pages/pos.tsx`): scan product → add to cart (disabled while the
    finalize/draft dialogs are open).
  - **Quotations** (`pages/quotations.tsx`) & **Purchases** (`pages/purchases.tsx`):
    scan product → add to the cart, **only while the "new" dialog is open** (the
    dialog component takes an `open` prop used as `enabled`).
  - **Sales** (`pages/sales.tsx`): scan an invoice serial → opens the invoice; the
    search box also opens on manual type + Enter.

---

## 8. Troubleshooting — likely problems & fixes

| Symptom | Cause / Fix |
|---|---|
| `No such built-in module: node:sqlite` | You're on Node 20. `nvm use 24`. |
| Backend `dev` builds then `start` crashes | The `start` step used a non‑24 Node. Ensure Node 24 is first on `PATH`. |
| `pnpm --filter @workspace/pos run typecheck` has errors | The OpenAPI spec lags the backend. Update `openapi.yaml` + run codegen. (libs + api‑server should be green.) |
| Tables inside a tab render left‑to‑right / headers not above data | `DirectionProvider` missing/removed, or a Radix wrapper without RTL. Check `App.tsx`. |
| Print shows the whole app / dialog frame / ✕ button | Check `@media print` in `index.css`: `#root{display:none}`, `[role=dialog]` reset, `.no-print` on overlay+close, invoice wrapped in `.invoice-print`. |
| Barcode doesn't render | jsbarcode draws into an `<svg>` ref inside `useEffect`; invalid values are caught silently. Verify the value is a valid Code128 string. |
| Scanner adds the wrong/no product | `/products/search?q=` matches `barcode` **exactly**; the product must have a `barcode`. `findProductByCode` prefers an exact barcode/SKU match. |
| Scanner triggers while typing by hand | Raise `minLength` or lower `maxGapMs` in `use-barcode-scanner.ts`. |
| Scanner fires when it shouldn't (closed dialog) | Use the hook's `enabled` option (quotations/purchases gate on `open`; POS gates on `!showFinalizeDialog && !showDraftDialog`). |
| Vite/esbuild won't start on macOS/Windows | Native binaries missing — make sure `pnpm-workspace.yaml` does **not** re‑add the platform‑exclude overrides, then `pnpm install`. (Old emergency workaround: set `ESBUILD_BINARY_PATH` to a fetched `@esbuild/darwin-arm64` binary.) |
| `pnpm install` adds an `allowBuilds:` block to `pnpm-workspace.yaml` | Harmless pnpm build‑script prompt noise — **revert it before committing**. |
| New import fails to resolve | pnpm strict mode — add the dep to that package's `package.json` and `pnpm install` (transitive deps are NOT importable). |
| Barcode stickers print a blank/extra page or wrong size | `lib/print-labels.ts` uses its own print window with `@page { size:40mm 15mm }`. The inline `<script>` must stay in `<head>` and the last‑page break uses `:last-of-type` — don't move them, or a trailing blank page returns. Popup blocked → save still works, a toast asks to allow popups. |
| `/products/next-barcode` returns 404 / “غير موجود” | The API wasn't restarted after the route was added, OR it's defined **after** `/products/:id` (it must be before). `dev` = `build && start`, so restart it. |

---

## 9. Open items / recommendations for the next agent

- **See §7 “▶ NEXT” for the live to‑do list** (sell‑by‑weight in the cashier is the top item).
- **The 2026‑06‑27 work is all UNCOMMITTED** — commit it once verified in the real app.
- **Adding a DB column?** Use the `ensureColumn(table, col, def)` migration helper in
  `lib/db.ts` (called at the end of `initializeSchema`). `CREATE TABLE IF NOT EXISTS` will
  **not** add a column to an existing DB, so add both the column in the `CREATE TABLE` (for
  fresh installs) **and** an `ensureColumn(...)` call (for existing installs). `soldByWeight`
  is the reference example.
- **Restart the API after backend edits** — `dev` = `build && start`, not watch.
- `data/store.db` holds the user's **live data** — don't reset it without asking.
- Consider gitignoring `data/store.db-shm` / `-wal` (transient; currently tracked → constant noise).
- The unused `lib/db` (Postgres/Drizzle) package could be deleted.
- Several POS pages still cast API responses `as any`; tighten them.
- Commission UI exists and works; keep it separate from debt.
